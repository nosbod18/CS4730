...
class Transforms():
    def __init__(self):
        ...
        self.pub = rospy.Publisher('/mover/cmd_vel', Twist, queue_size=1)
    ...
    def update_current(self):
        self.H_initial_to_current = np.array([[np.cos(self.RPY_current[2]), -np.sin(self.RPY_current[2]), 0, self.position_current.x],
                                              ...
                                              [0,                            0,                           0, 1]])
        self.H_current_to_goal = np.dot(np.linalg.inv(self.H_initial_to_goal), self.H_initial_to_current)

        self.imu_received = False
        self.odom_received = False


    def move(self, x_G, y_G, theta_G):
...
if __name__ == '__main__':
    ...
    transforms.move(.5, .5, 90)
    transforms.move(-.5, .5, 180)
    transforms.move(-.5, -.5, -90)
    transforms.move(.5, -.5, 0)
