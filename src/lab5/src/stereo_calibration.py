...
def calibrate():
    ...
    f.write('K_l', K_l)
    f.write('P_l', P_l)
    f.write('dist_l', dist_l)
    f.write('K_r', K_r)
    f.write('P_r', P_r)
    f.write('dist_r', dist_r)
...
def load_calibration():
    ...
    return P_l, K_l, dist_l, P_r, K_r, dist_r
...
P_l, K_l, dist_l, P_r, K_r, dist_r = load_calibration()

print(P_l)
print('\n')

uv_l = find_uv('images/detection/left03.ppm')
uv_r = find_uv('images/detection/right03.ppm')

M = find_xyz(uv_l, uv_r, P_l, P_r)
M = np.array([M[0], M[1], M[2], 1])
